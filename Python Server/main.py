...
LDAPServerName = 'esss.lu.se'
LDAPServer = ldap3.Server(LDAPServerName)

PORT_NUMBER = 4040

usersFile = "./users.json"
servicesFile = './services.json'
userBaseDir = "/var/www/podcast/push/usersfeeds"
...
def sendPushToDevice(apn, message):
    token = jwt.encode( { 'iss': TEAM_ID, 'iat': time.time() },
        APNS_AUTH_KEY,
        algorithm= ALGORITHM,
        headers={ 'alg': ALGORITHM, 'kid': APNS_KEY_ID })
    request_headers = { 'apns-expiration': '0', 'apns-priority': '10', 'apns-topic': BUNDLE_ID, 'authorization': 'bearer {0}'.format(token.decode('ascii')) }
    for server in APPLE_SERVER:
        conn = HTTP20Connection(server, force_proto='h2')
        path = '/3/device/{0}'.format(apn)
        conn.request('POST', path, message, headers=request_headers)
...
def createUserFolder(userData):
    ...
    if not os.path.isfile(userBaseDir+'/'+userData["ESSUser"]+'/notifications.json'):
        emptydata = {userData["ESSUser"]: []}
        saveObj(emptydata, userBaseDir+'/'+userData["ESSUser"]+'/notifications.json')
    if not os.path.isfile(userBaseDir+'/'+userData["ESSUser"]+'/services.json'):
        services = loadObj(servicesFile)
        emptydata = {"services": []}
        for service in services["services"]:
            emptydata["services"].append( {"id": service['id'], "Category": service['Category'], "Color": service['Color'], "Subscribed": False})
        saveObj(emptydata, userBaseDir+'/'+userData["ESSUser"]+'/services.json')
...
def regenerateServicesFiles():
    ...
    for user in userData:
        ...
        for service in servicesData["services"]:
            emptydata["services"].append( {"id": service['id'], "Category": service['Category'], "Color": service['Color'], "Subscribed": user in service['Subscribers']})
...
def auth_handler(userData: dict):
    if not userData["Registered"]:
        try:
            LDAPConnection = ldap3.Connection(LDAPServer, userData["ESSUser"]+"@esss.se", userData["ESSToken"])
        ...
        if userData["ESSUser"] in users and userData["ESSToken"] == users[userData["ESSUser"]]["ESSToken"]:
            userData["Registered"] = True
            addUser(userData)
        else:
            userData["Registered"] = False
...
def services_handler(serversData: dict):
    ...
    for service in serversData["services"]:
        ...
        if service['Subscribed'] and (serversData['user'] not in serviceslist['services'][service['id']]['Subscribers']):
            serviceslist['services'][service['id']]['Subscribers'].append(serversData['user'])
        if not service['Subscribed'] and (serversData['user'] in serviceslist['services'][service['id']]['Subscribers']):
            serviceslist['services'][service['id']]['Subscribers'].remove(serversData['user'])

    saveObj(userservices, userBaseDir+'/'+serversData['user']+'/services.json')
...
def updateNotifications_handler(updateNotificationsData: dict):
    ...
    saveObj({updateNotificationsData['user']: updateNotificationsData['notifications']}, userBaseDir+'/'+updateNotificationsData['user']+'/notifications.json')
    return {}
...
def newpush_handler(pushData: dict):
    ...
    try:
        for user in usersrecv:
            ...
            notifications[user].append({"id": msgid,
    ...
    try:
        for i in notifications[user]:
            if not i['Read']:
                current_badge+=1
        payload_data = { 'aps': { 'alert' : { "title": pushData[token]["Title"], "subtitle": pushData[token]["Message"] }, 'badge': current_badge, 'sound' : 'default' } }
